...
<?php
  ...
  $url = $_GET['url'];
  $file = file_get_contents($url); // 此範例讀取的是 ZIP 檔，為原始的二進位資料

  header('Content-Type: application/zip');    // 告知瀏覽器該內容是 ZIP 檔
  header('Content-Length: ' . strlen($file)); // 回傳內容大小，單位是 bytes
  echo $file;
?>
```
...
<figure><img src="../.gitbook/assets/diagonal_stream_cdn_video.gif" alt=""><figcaption></figcaption></figure>
