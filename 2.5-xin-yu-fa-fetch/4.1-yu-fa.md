...
## 認識 Promise chaining(鏈)

可連續使用多個 then：

{% embed url="https://codepen.io/carlos411/pen/dPXvKbY" %}
...
一、以下語法當中的**第一個 then** 的箭頭函式：得到的是一個 **Response 的物件**，主要包含**表頭資訊(headers)及其它**，透過 `res.ok` 可用來**判斷跟遠端的伺服器是不是連線都正常**。如果 res.ok 得到 true，再透過 **`return res.json()`** 函式，來解析後端的資料並回傳，然後資料會帶到下一個 then。
...
* **`new FormData`**：Content-Type 會自動是 「 **`multipart/form-data`** 」，可傳遞檔案類型。
* **`new URLSearchParams(FormData)`**：Content-Type 會自動是 「 **`x-www-form-urlencoded`** 」，不可傳遞檔案類型。
