        ...
        $("#the_file").on("change", function(e){
          ...
          for (let i = 0; i < this.files.length; i++) {
            ...
            reader.addEventListener("load", function (e) {
              ...
              $.ajax({
                url: "http://notes.webmix.cc/html5_tutorial/file/file_receive.php",
                type: "POST",
                data: form_data,
                dataType: "json",
...
執行 `http://localhost:3000/practice/upload_file.html` 測試看看。
...
建立 `ajax/practice/upload_file_single.html` 檔案，並用本機端網址方式開啟。

撰寫的重要步驟：

1. 寫一個 input 標籤，type 類型為 file。
2. ，監聽 change 事件的發生，然後使用 new FormData() 來存放資料：`let form_data = new FormData();`
3. 資料的名稱叫做 the\_file：`form_data.append('the_file', this.files[i]);`
4. 撰寫 ajax 將資料傳送到 `http://notes.carlos-studio.com/html5_tutorial/file/file_receive.php` 網址。



參考作法：(以下範例無法在 CodePen 執行，因為 Mixed Content 的關係)

{% embed url="https://codepen.io/carlos411/pen/ZEJbJGR?editors=1010" %}
...
建立 `upload_file_single_fetch.html` ，貼以下的原始碼，執行看看：
    ...
    <script>
      ...
      $(function(){
        ...
        $("#the_file").on("change", function(){
          ...
          // 寫法一：
          fetch("http://notes.webmix.cc/html5_tutorial/file/file_receive.php", {
            method: "POST",
            body: form_data
          ...
          // 寫法二：
          /*
          fetch("http://notes.carlos-studio.com/html5_tutorial/file/file_receive.php", {
            method: "POST",
            body: form_data
