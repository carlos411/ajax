...
{% swagger baseUrl="http://notes.carlos-studio.com" path="/ajax/teach/api/list.php" method="get" summary="取得待辦事項列表" %}
{% swagger-description %}
取得待辦事清的清單列表。
...
{% swagger-response status="403" description="禁止取得待辦事項的 list。" %}
```javascript
{
  msg: "denied"
}
...
{% swagger baseUrl="http://notes.carlos-studio.com" path="/ajax/teach/api/add.php" method="post" summary="新增待辦事項" %}
{% swagger-description %}
輸入欲新增的待辦事項。
{% endswagger-description %}
...
{% swagger-response status="200" description="回傳新增的待辦事項資料。
item_id: 伺服器回傳的待辦事項 id。" %}
```javascript
{
  "item_id": "abc123",
  "name":"dddd",
...
{% swagger baseUrl="http://notes.carlos-studio.com" path="/ajax/teach/api/delete_item.php" method="delete" summary="移除單筆待辦事項" %}
{% swagger-description %}
移除單筆待辦事項 
{% endswagger-description %}
...
{% swagger-parameter in="body" name="item_id" type="string" %}
待辦事項的 id。
{% endswagger-parameter %}

{% swagger-response status="200" description=" 回傳成功訊息(包含資料)，或 待辦事項找不到或未定義" %}
```javascript
/* 刪除待辦事項成功 */
{
  "msg":"delete success",
...
{% swagger-response status="403" description="未使用 DELETE 或其它原因。" %}
```javascript
{
  msg: "denied"
}
...
{% swagger baseUrl="http://notes.carlos-studio.com" path="/ajax/teach/api/delete_all.php" method="delete" summary="清空所有待辦事項" %}
{% swagger-description %}
清空所有待辦事項。
{% endswagger-description %}
...
{% swagger baseUrl="http://notes.carlos-studio.com" path="/ajax/teach/api/patch_star.php" method="patch" summary="更新待辦事項的星號" %}
{% swagger-description %}
更新待辦事項的星號。
{% endswagger-description %}
...
{% swagger-parameter in="body" name="star" type="string" %}
星號數字。
{% endswagger-parameter %}
...
{% swagger baseUrl="http://notes.carlos-studio.com" path="/ajax/teach/api/patch_sort.php" method="patch" summary="更新待辦事項的排序" %}
{% swagger-description %}
...
{% swagger baseUrl="http://notes.carlos-studio.com" path="/ajax/teach/api/update_item.php" method="put" summary="更新單筆待辦事項全部資料" %}
{% swagger-description %}
更新的資料包含待辦事項文字、星號、排序。
...
(這裡是為了讓同學也試試 PUT 方式來更新整體資料，不然其實可以用 PATCH 只更新文字部份。)
{% endswagger-description %}
...
{% swagger-parameter in="body" name="star" type="integer" %}
星號數。
{% endswagger-parameter %}

{% swagger-parameter in="body" name="sort" type="integer" %}
排序數值。
{% endswagger-parameter %}
...
[https://alldata.sgp1.digitaloceanspaces.com/sample/assignment\_number\_ajax.zip](https://alldata.sgp1.digitaloceanspaces.com/sample/assignment\_number\_ajax.zip)
